...
from caic.constants import BOLD_RED, BOLD_GREEN, BOLD_YELLOW, BOLD_BLUE, BOLD_MAGENTA, BOLD_CYAN, NORMAL, NEW_LINE
from caic.constants import SLEEP_0250_MS
from caic.utilities import logger
from caic.utilities import model
from caic.utilities.processor import execute_synchronous, execute_synchronous_unregistered
...
def exited_virtual_environment(process_id, status, pseudo_terminal):
    ...
    #   $ pkexec /usr/share/caic/commands/stop-process <pid of start-console>
    #   status = 9
    #
    # • Execute the following from outside Cubic's terminal.
...
def is_virtual_environment_running():
    ...
    command = 'machinectl --property=State show caic'
    result, exit_status, signal_status = execute_synchronous(command)
